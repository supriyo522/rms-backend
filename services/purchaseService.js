...
const savePurchases = async (purchaseData) => {
  const purchases = purchaseData.map(item => {
    let purchase = {};
    if (item['Bill Date']) {
      const [day, month, year] = item['Bill Date'].split('/');
      purchase.billDate = new Date(year, month - 1, day);
    }
...
const getAllPurchases = async () => {
  ...
  // // Fetch sections from Section collection
  // return await Section.find({});
  

  // // Combine purchases and sections
  // const combinedData = [...purchases, ...sections];

  // return combinedData;
};

const getSalePersonPerform = async () => {
  // return await Purchase.find({}, {salesManName: 1, sellQuantity: 1, finalScore: 1, closingQTY: 1, closingAMT: 1 });
  // return purchases.filter(purchase => Object.keys(purchase).length !==0);
  try {
    const results = await Purchase.find(
      { 
        salesManName: { $exists: true }, 
        sellQuantity: { $exists: true }, 
        finalScore: { $exists: true }, 
        closingQTY: { $exists: true }, 
        closingAMT: { $exists: true } 
      },
      { 
        _id: 0, 
        salesManName: 1, 
...
const getSaleForcast = async (date) => {
  const query = date ? { billDate: new Date(date) } : {};
  return await Purchase.find(query, { _id: 0, division: 1, section: 1, department: 1, itemCode: 1});
};

const getStockAtGlance = async () => {
  return await Purchase.find({}, { _id: 0, division: 1, section: 1, department: 1, itemCode: 1, vendor: 1, barcode: 1, grAmt: 1, cat1: 1, rsp: 1, closingQTY: 1, closingAMT: 1 });
};
